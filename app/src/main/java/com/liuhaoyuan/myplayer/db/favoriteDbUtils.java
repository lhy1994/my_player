...
public class FavoriteDbUtils {
    ...
    private static FavoriteDbUtils instance;
    ...
    private FavoriteDbUtils(Context context) {
        mHelper = new FavoriteOpenHelper(context);
    }

    public static synchronized FavoriteDbUtils getInstance(Context context) {
        if (instance == null) {
            instance = new FavoriteDbUtils(context);
        }
        return instance;
    }
    ...
    public ArrayList<NetVideoItem> queryAllVideo() {
        SQLiteDatabase database = mHelper.getWritableDatabase();
        Cursor cursor = database.query(VIDEO_TABLE_NAME, null, null, null, null, null, null);
        ArrayList<NetVideoItem> list = new ArrayList<>();
        if (cursor != null) {
            while (cursor.moveToNext()) {
                ...
                try {
                    ...
                    NetVideoItem videoItem = (NetVideoItem) objectInputStream.readObject();
                    list.add(videoItem);
                } catch (IOException e) {
                    e.printStackTrace();
    ...
    public void insertVideo(NetVideoItem videoItem) {
        byte[] bytes = videoInfoToByteArray(videoItem);
        if (bytes == null) {
            return;
        }
        ContentValues values = new ContentValues();
        values.put(VIDEO_ID, videoItem.programmeId);
        values.put(VIDEO_INFO, bytes);

        SQLiteDatabase database = mHelper.getWritableDatabase();
        Cursor cursor = database.query(VIDEO_TABLE_NAME, null, VIDEO_ID + "=?", new String[]{videoItem.programmeId}, null, null, null);
        if (cursor != null && cursor.getCount() > 0) {
            database.update(VIDEO_TABLE_NAME, values, VIDEO_ID + "=?", new String[]{videoItem.programmeId});
            cursor.close();
        } else {
    ...
    private byte[] videoInfoToByteArray(NetVideoItem videoItem) {
